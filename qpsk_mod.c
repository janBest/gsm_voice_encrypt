...
/*
	...
	{
		printf("%f %lf\n",t+2,sinc_interpolation(t+2,smp_points,input_data_len));
	}
*/
	for(t=0;t < (input_data_len-1)*SAMPLING_INTERVAL;t+= UNIT_TIME*2)
	{
	...
	//	fprintf(fp2,"%f %lf\n",t,sinc_interpolation(t,smp_points,input_data_len));
		T1 = floor(t/T) *T; 
		T2 = T1+T;
		a = integral(i_f,T1,T2,100,smp_points,input_data_len);
		b = integral(q_f,T1,T2,100,smp_points,input_data_len);
		if(a>0 && b>0)
		{
			printf("00");
		}
...
double sinc_interpolation(float t,const double *smp_points,const int l)
{
	...
	for(j=n-a;j<(n+a+2);j++)
	{
		...
		if(j >= l)
			continue;
		temp=sin((t-j*SAMPLING_INTERVAL)*PI)/(PI*(t-j*SAMPLING_INTERVAL));
...
int ad_simulator(PSIGNAL_FUNC s,void *input_data,const int* input_data_len,void *output_data,int *output_data_len)
{
	...
	for(i =0; i<(*input_data_len) * UNIT_TIME; i+= SAMPLING_INTERVAL)
	{
		...
		fprintf(fp2,"%f %lf\n",i,s(i,input_data,*input_data_len));
...
double qpsk_s(float i,void *input_data,int input_data_len)
{
	...
	int n1 = get_a_bit((void*)input_data,i1);
	int n2 = get_a_bit((void*)input_data,i2);


	return cos(2*PI*FREQUENCY*i/1000+phase_map[n1][n2]);
}
...
double pcm_decode(int V,char data)
{
	...
	double mc;
	//所在段落   
	int DL=get_a_bit((void *)&data,6)*4 + get_a_bit((void *)&data,5)*2 + get_a_bit((void *)&data,4)+1;
    //所在段内 
	int DN=get_a_bit((void *)&data,3)*8 + get_a_bit((void *)&data,2)*4 + get_a_bit((void *)&data,1)*2+ get_a_bit((void *)&data,0) +1;
	switch(DL)   
    {   
	 ...
	 mc=mc*V/2048;   
    if(get_a_bit((void *)&data,7) ==0)  mc=mc*(-1);
	return mc;
}
